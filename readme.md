...
DP optimizer), saves the result to the database, and returns a
`prediction_id` (int). **Every call creates a new row — nothing is
overwritten.** Store this id if the dashboard needs to link back to a
specific run later (e.g. "show me the run I just made").
...
the math (per-leg splits, cumulative fuel, formatted labels) was
already done once at prediction time and stored flat.
...
| `savings_label` | str | `'3.4% saved'` or `'Infeasible'` | ready to drop into a card, no formatting needed |
| `is_feasible` | int (0/1) | `1` | check this before showing `savings_pct`-based charts |
| `n_legs` | int | `10` | |
| `total_dist_nm` | float | `3149.46` | |
| `created_at` | str (ISO 8601, UTC) | `'2026-09-05T17:04:46...'` | when this run happened |
...
| `baseline_fuel_t` | float | this leg's *share* of the baseline total (split proportionally by distance) — use for a DP-vs-baseline comparison bar chart |
| `cum_dp_fuel_t` | float | running total — plug directly into a "fuel burned so far" line chart, no cumsum needed |
| `hs_m` | float or `None` | wave height overlay |
| `wind_speed_ms` | float or `None` | wind overlay — will be `None` for wave-only (non-wind) runs' weather diagnostics in some cases; always check for `None` before plotting |
...
- Don't recompute fuel/savings numbers client-side — they're already
...
- Don't call `run_prediction()` on every page load — it re-runs the
  ...
  the user explicitly requests a new prediction (e.g. clicks "Run" or
  changes route/period/ballast inputs).
